...
function isLikelyMessage(text) {
  ...
  const uiPatterns = [
    /^You sent/i, /^Active now/i, /^Active \d/i,
    /^Send message/i, /^GIF$/, /^Sticker$/, /^Message$/,
    /^\d+[mh]$/, /^Yesterday$/, /^Today$/, /^Just now$/,
    /^Seen$/, /^Delivered$/, /^Sending\.\.\.$/
  ];
...
function markExistingMessagesAsSeen() {
  ...
  for (const div of area.querySelectorAll('div[dir="auto"]')) {
    ...
    const text = div.textContent.trim();
    if (text) { recentlyProcessed.set(text, Date.now()); count++; }
  }
...
function extractConversationHistory() {
  ...
  if (!area) return [];
  const history = [];
  const divs = Array.from(area.querySelectorAll('div[dir="auto"]'))
    .filter(d => isLikelyMessage(d.textContent.trim()));

  for (const div of divs.slice(-16)) { // Last 16 messages max
    const text = div.textContent.trim();
...
async function typeIntoInput(text) {
  ...
  if (!input) { log('ERROR: Message input not found'); return false; }
  ...
  // Primary method: execCommand insertText
  // This triggers the correct browser events that Lexical/React listens to
  const ok = document.execCommand('insertText', false, text);

  if (ok && input.textContent.includes(text.substring(0, Math.min(20, text.length)))) {
    return true;
  }

  // Fallback: ClipboardEvent paste simulation
  log('execCommand failed, trying clipboard paste fallback...');
  input.innerHTML = '';
  ...
  // Last resort: direct content + synthetic events
  log('Clipboard fallback failed, using direct content method...');
...
async function callGroq(messages) {
  return new Promise((resolve, reject) => {
    chrome.runtime.sendMessage(
      { type: 'CALL_GROQ', payload: { apiKey: settings.apiKey, systemPrompt: settings.systemPrompt, messages } },
      (response) => {
...
async function handleNewMessage(messageText) {
  ...
  try {
    ...
    if (!reply) { log('Empty reply from Groq.'); return; }
    log('Got reply:', reply);

    const typed = await typeIntoInput(reply);
    if (!typed) { log('ERROR: Could not type reply into input.'); return; }
...
function init() {
  ...
  const poller = setInterval(() => {
    ...
    if (area && input) {
      ...
      clearInterval(poller);
      log('Timed out waiting for the messages page to fully load. Try refreshing.');
    }
  }, 1000);
}
...
function log(...args) { console.log('[FB AutoReply]', ...args); }
